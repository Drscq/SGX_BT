...
typedef struct ms_ecall_early_reshuffle_2_t {
	char* ms_buffer;
	uint8_t* ms_flags;
} ms_ecall_early_reshuffle_2_t;
...
typedef struct ms_ecall_evict_2_t {
	char* ms_buffer;
	uint8_t* ms_flags;
} ms_ecall_evict_2_t;
...
typedef struct ms_ecall_test_bignum_t {
	char* ms_result;
	size_t ms_result_len;
} ms_ecall_test_bignum_t;
...
{
	sgx_status_t status;
	ms_ecall_write_to_untrusted_t ms;
	ms.ms_data = data;
	ms.ms_data_len = data_len;
...
sgx_status_t ecall_early_reshuffle_2(sgx_enclave_id_t eid, char* buffer, uint8_t* flags)
{
	sgx_status_t status;
	ms_ecall_early_reshuffle_2_t ms;
	ms.ms_buffer = buffer;
	ms.ms_flags = flags;
...
sgx_status_t ecall_evict_2(sgx_enclave_id_t eid, char* buffer, uint8_t* flags)
{
	sgx_status_t status;
	ms_ecall_evict_2_t ms;
	ms.ms_buffer = buffer;
	ms.ms_flags = flags;
